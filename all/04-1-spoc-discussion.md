...
从左往右：容量变大，延迟变高
寄存器可以直接寻址，（高速缓存全透明不能寻址），内存可以寻址（mov里只能出现一次），外存必须借助io完成

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

均衡繁忙：不存在一个层次成为整个系统的效率瓶颈

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

有（组成原理开发板的内存严重不足）：拿外存储手写swap、升级内存

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

覆盖技术：同一个程序不同组成部分在不同时间占用内存 主要工作：决定不同部分进入内存的时序

2. 什么是交换技术？覆盖与交换有什么不同？

交换技术：进程的内存换入或换出硬盘。
覆盖解决进程内的问题，交换解决进程间的问题
...
时间局部性：同一个位置会在不久的将来被访问
空间局部性：附近的位置会在不久的将来被访问
分支局部性：接下来程序控制流的运行很可能和现在一致
...
虚拟存储：把用到的部分放进物理内存
好处：应用程序实现简单，不需要考虑覆盖问题
挑战：实现复杂

2. 虚拟存储需要什么样的支持技术？

硬件虚拟地址转换；一个较好的页面置换算法

### 8.5 虚拟页式存储

 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？

虚拟页式存储：以页为单位，将程序需要的内存在物理内存和硬盘之间交换
缺页中断：将需要的页换入内存

 1. 为了支持虚拟页式存储的实现，页表项有什么修改？

需要有valid、dirty（可以用write位模拟）、read（可以用access位模拟）等位

 2. 页式存储和虚拟页式存储的区别是什么？

虚拟页式存储基于页式存储，强调和硬盘的swap操作

### 8.6 缺页异常

1. 缺页异常的处理流程？

缺页trap被触发，操作系统检查确认是缺页，执行io操作把页面换入内存，继续执行
